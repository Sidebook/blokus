    ...
    fn handle(&mut self, msg: Result<ws::Message, ws::ProtocolError>, ctx: &mut Self::Context) {
        ...
        match msg {
            ...
            ws::Message::Text(text) => {
                ...
                if player_id == -1 {
                    ctx.text(format!["Invalid request: Player ID is not defined {}", &text]);
                    return;
                }
                ...
                match caps.next() {
                    Some(command) => {
                        match command.as_str() {
                            ...
                            _ => {
                                ctx.text(format!["Invalid request: unknown command {}", command.as_str()]);
                            }
                        }
                    },
                    None => {
                        ctx.text(format!["Invalid request: no command"])
                    },
                }
                // }
                // caps.at(1).unwrap()
...
async fn echo_route(req: HttpRequest, stream: web::Payload, ism_data: Data<Mutex<InputQueue>>) -> Result<HttpResponse, Error> {
